...
FO=1 inverter pair ($5\times$ input, $6\times$ driver, unit
$440/\SI{220}{\nano\meter}$); fixed MOSCAP ($18\times$ fingers) behind an
always-on access device ($7\times$); three TG-switched trim banks of
$2/20/21$ fingers. \\
\texttt{delay\_tau2} & 7 &
...
$\ttau{2}$. Equal input and driver sizing ($5\times$ and $6\times$ unit
...
three binary-weighted-like banks ($2/20/21$ fingers on the $\ttau{1}$ cell,
$2/14/18$ on the $\ttau{2}$ cell) are switched in or out through transmission
gates by the static \texttt{trim<0:2>} inputs. Because the $6{:}5$ delay ratio
comes from loading rather than device sizing, $\ttau{1}$ and $\ttau{2}$ track
...
line across the top, STOP line down the left, and the $10\times10$ padded
