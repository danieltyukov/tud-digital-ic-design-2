...
$440/\SI{220}{\nano\meter}$); MOSCAPs for trimming: three TG-switched trim banks of
...
capacitively (Fig.~\ref{fig:delaycells}). Sizing it is a compromise. Larger
devices average out mismatch and make the stage less sensitive to parasitics
and load variation, which protects linearity and corner stability; the same
width, however, costs area and power and adds self-loading, which slows the
stage and can push it outside the usable delay window. The chosen size is
therefore the smallest one that still meets the \SIlist{75;90}{\pico\second}
targets under the true grid load of Section~\ref{sec:loading}.

The internal fan-out follows the same logic. Conventional FO=4 sizing
minimizes the delay of a logic path, but here delay is the product, not the
cost: with FO=4 the intrinsic delay of the internal node pins the minimum
achievable stage delay at \SI{97}{\pico\second} in this technology, beyond
the required $\ttau{2}$. Equal input and driver sizing ($10\times$ and
$10\times$ unit fingers, FO=1) reduces the internal-node capacitance and
opens a usable window of \SIrange{74}{147}{\pico\second} under the true grid
load.

Trimming is capacitive, with identical actives. Both lines use the same
active devices; only the capacitance on the internal node differs. The fixed
MOSCAP ($18\times$ fingers, $L=\SI{250}{\nano\meter}$) sets the nominal
delay, and three banks ($2/20/21$ fingers on the $\ttau{1}$ cell, $2/14/18$
on the $\ttau{2}$ cell) are switched in or out through transmission gates by
the static \texttt{trim<0:2>} inputs. Because both lines share the same
$10\times$ FO=1 actives, $\ttau{1}$ and $\ttau{2}$ track each other over
process and temperature, which is what keeps their difference, the LSB,
stable. At the trimmed typical-corner point the lines measure
