...
dummy latches fill the unused grid cells so the load is uniform along each
line. Buffer trees distribute
...
same delay, and stage delay depends on load. With only the 31 mapped latches
in place, the taps would drive anywhere from one to ten latch inputs, which
would modulate the stage delays along each line and corrupt the thresholds.
We therefore fill the unused cells of the $10\times6$ array with 29 dummy
latches, electrically identical to the active ones but with unused outputs:
every START tap then drives exactly six latch inputs and every STOP tap
exactly ten, so the load is uniform along each line. The same argument adds
one extra delay cell at the head of each line (11 and 7 cells in total), so
that the first logical stage sees the same driver as all others.
