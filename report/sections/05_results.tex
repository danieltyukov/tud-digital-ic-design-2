...
configuration (the tuned TT setting) in all five corners; Table~\ref{tab:corners}
...
\SI{-24}{\pico\second}, constant per corner, and is a code offset rather than
a linearity error.
...
\toprule
\textbf{Corner} & {\textbf{LSB} (\si{\pico\second})} & {\textbf{DNL$_{\max}$} (LSB)} &
{\textbf{INL} (LSB)} & {\textbf{Offset} (\si{\pico\second})} &
{\textbf{E/conv} (\si{\pico\joule})} & {\textbf{ENOB}} &
{\textbf{FoM} (\si{\pico\joule}/step)} \\
\midrule
...
all other corners stay at or below \num{0.80}. The dominant DNL
feature is a systematic sawtooth with period 6: peaks of \numrange{0.6}{0.7}
LSB at codes 7, 13, 19 and 25, exactly the column-wrap boundaries of the
mapping in Eq.~\eqref{eq:mapping}, where the threshold hands over from a row
step to a column step. The pattern is identical in every corner, which
identifies it as an architectural path-length imbalance at the wrap rather
than device mismatch. Calibration does not remove it
(Section~\ref{sec:calibration}); a targeted per-wrap capacitive correction
could, and is listed as future work.
...
\begin{subfigure}[t]{0.49\textwidth}
  ...
  \caption{LSB per corner, fixed trim vs calibrated.}
  \label{fig:lsbtuned}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}
  \includegraphics[width=\textwidth]{figures/calibration_range.png}
  \caption{Calibration authority: stage-level $\tlsb$ vs engaged capacitance
  imbalance, all 64 configurations in 5 corners.}
  \label{fig:calrange}
\end{subfigure}
\caption{MOSCAP-bank calibration. Every corner re-centers to the
...
(worst tuned INL \num{0.74} LSB), as expected: calibration re-centers the
average step but cannot touch the architectural wrap pattern. One honest data
point: the FS optimum is the default configuration, since that corner shifts N
and P devices oppositely and nearly cancels. All sign-off checks still pass in
every corner.
...
netlist. Because calibration only changes gate biases, this number is
identical in all five corners. Including the testbench interface cells would
...
\SI{1}{\femto\second}, four orders of magnitude under the LSB. Two caveats
apply: the bound is grid-limited, and the run contains no noise or mismatch,
so a Monte-Carlo analysis would widen it. Both qualifications leave orders of
magnitude of margin.
...
1-D Vernier row (\texttt{vernier2d/vernier1d\_row}) from the very same delay
and latch cells, with trim banks hardwired on and, deliberately, no fan-out
...
\SI{16.6}{\pico\second}: direct experimental confirmation that unequalized tap
loading roughly doubles the LSB, the effect that drove the sizing methodology
of Section~\ref{sec:loading}. Per code, the 2-D core is about twice as
area-efficient at comparable energy. Extrapolating the row to the full 31
codes would require 62 delay stages ($3.4\times$),
$\sumw\approx\SI{2524}{\micro\meter}$ ($1.9\times$) and a full-scale latency
of \SI{3.3}{\nano\second} against \SI{0.9}{\nano\second} for the grid.
